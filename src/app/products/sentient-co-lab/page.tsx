    ...
    <>
      ...
      <main>
        ...
        <section className="bg-black pt-[4.5rem] text-white">
          <Container>
            <div className="grid min-h-[calc(100svh-4.5rem)] gap-12 py-12 lg:grid-cols-[1.06fr_0.94fr] lg:items-center lg:py-20">
        ...
        <section id="capabilities" className="scroll-mt-20 bg-[#f6f6f1] text-black">
          <Container className="py-20 sm:py-28">
            ...
            <ul className="mt-12 grid gap-x-12 gap-y-6 border-t border-black/15 pt-8 sm:grid-cols-2 lg:grid-cols-3">
              {capabilities.map((item) => <li key={item} className="flex gap-3 text-sm font-semibold leading-6"><span className="text-[#1f5eff]">•</span>{item}</li>)}
            </ul>
          </Container>
        </section>
        ...
        <section className="bg-black text-white">
          <Container className="py-24 sm:py-32">
            <div className="grid gap-16 lg:grid-cols-[0.8fr_1.2fr]">
              <div><p className="eyebrow text-[#83a8ff]">How the product fits together</p><h2 className="mt-7 text-5xl font-medium leading-none tracking-[-0.05em] sm:text-6xl">Real-time interaction. Durable knowledge.</h2><p className="mt-6 max-w-xl text-base leading-8 text-white/55">Co-Lab treats communication, collaborative work and AI knowledge capture as connected product layers rather than separate tools.</p></div>
              <ul className="border-t border-white/20">{architecture.map(([title, copy]) => <li key={title} className="grid gap-5 border-b border-white/20 py-8 sm:grid-cols-[0.75fr_1.25fr]"><h3 className="text-xl font-semibold">{title}</h3><p className="text-sm leading-7 text-white/50">{copy}</p></li>)}</ul>
            </div>
          </Container>
        </section>
        ...
        <section className="bg-white text-black">
          <Container className="py-20 sm:py-28">
            ...
            <div className="mt-10 grid gap-x-10 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">{useCases.map((item) => <div key={item} className="flex gap-3 border-t border-black/15 pt-5"><span className="text-[#1f5eff]">•</span><p className="text-sm font-semibold leading-6">{item}</p></div>)}</div>
