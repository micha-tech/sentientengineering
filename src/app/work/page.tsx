      ...
      <main>
        ...
        <section className="bg-black pt-[4.5rem] text-white">
          <Container className="border-x border-white/15 px-0">
            <div className="grid min-h-[38rem] lg:grid-cols-[1.15fr_0.85fr]">
              ...
              <div className="grid grid-cols-2">
                {["Problem", "System", "Status", "Value"].map((item, index) => <div key={item} className="flex min-h-48 flex-col justify-between border-b border-r border-white/15 p-5"><span className="text-xs font-bold text-white/25">0{index + 1}</span><p className="text-xl font-semibold">{item}</p></div>)}
              </div>
            </div>
          </Container>
        </section>
        ...
        <section className="bg-[#f6f6f1] text-black">
          <Container className="border-x border-black/10 px-0">
            {workEntries.map((entry, index) => (
              <article key={entry.id} id={entry.id} className="scroll-mt-24 border-b border-black/10">
                <div className="grid lg:grid-cols-[0.78fr_1.22fr]">
                  <div className="border-b border-black/10 p-6 py-12 sm:p-10 lg:border-b-0 lg:border-r">
                    ...
                    <span className="mt-7 inline-flex border border-black/15 bg-white px-3 py-2 text-[0.62rem] font-bold uppercase tracking-[0.09em]">{entry.status}</span>
                  </div>
                  <div className="p-6 py-12 sm:p-10">
                    ...
                    <div className="mt-9 grid gap-8 border-t border-black/15 pt-8 sm:grid-cols-2">
                      ...
                      <div><p className="eyebrow text-black/35">Relevant capabilities</p><ul className="mt-4 flex flex-wrap gap-2">{entry.capabilities.map((item) => <li key={item} className="border border-black/15 px-3 py-2 text-xs font-semibold">{item}</li>)}</ul></div>
                    </div>
            ...
            <div className="p-6 py-16 sm:p-10 sm:py-20">
